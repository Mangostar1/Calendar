...
const nameMonth = [//<-- Usend on some functions.
  "Enero", 
  "Febrero", 
  "Marzo", 
  "Abril", 
...
export function startDay() {//<-- Correct the day the month starts
  let start = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1);
  return ((start.getDay() - 1) === -1) ? 6 : start.getDay() - 1;
}

export function startDayYear(month) {//<-- Corrects the day the month begins in the year
...
document.getElementById("date-month").innerHTML = nameMonth[currentDate.getMonth()] + " de " + currentDate.getFullYear();
...
document.addEventListener('click', ({target}) => {
  ...
  if (target.matches('.menu-img')) {//<-- Mobile components control
  ...
  if (target.matches('#radio-day')) {//<-- Day
    ...
    document.getElementById("date-day").innerHTML = currentDate.getDate() + " de " + nameMonth[currentDate.getMonth()] + " del " + currentDate.getFullYear();
  }
  if (target.matches('#radio-week')) {//<-- Week
    ...
    document.getElementById("date-week").innerHTML = nameMonth[currentDate.getMonth()] + " de " + currentDate.getFullYear();
  }
